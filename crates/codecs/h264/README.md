...
the normative in-loop deblocking filter reconstruct nonzero luma and chroma residuals for profiles
with implicit flat scaling matrices. The CAVLC P-slice path retains one list-0 reference and
...
boundary strengths. Both Baseline and the High Profile subset using CAVLC, implicit flat scaling,
and 4x4 transforms are covered. The native CABAC arithmetic engine now handles context
initialization, decision, bypass, termination, and restart operations, with an exact x264 CABAC
`I_PCM` reconstruction path as its first integrated slice. This is a
normative pixel path covering parameter activation, slice traversal, prediction, reference
retention, macroblock placement, filtering, cropping, colour metadata, and packet timing. Custom
custom scaling matrices, general CABAC macroblocks, 8x8 transforms/`Intra_8x8`, B slices, multiple-reference decoded-picture-buffer
